...
def index():
    data = load_data()
    ratings = load_ratings()  # {row_index: [ {rating: X, comment: "..."} , ... ]}
    # Prepare summary averages
    ratings_summary = {}
    for k,v in ratings.items():
        avg = calculate_average(v)
        count = len(v)
        ratings_summary[k] = {'average': avg, 'count': count}
    ...
    <head>
      ...
      <title>Calificación y Comentarios por Destino - Detalle</title>
      <style>
        body {
    ...
    <body>
      ...
      const commentsData = {{ ratings|tojson }};
      ...
      function openModal(key) {
        modalBody.innerHTML = '';
        const entries = commentsData[key] || [];
        if (entries.length === 0) {
    ...
    """
    return render_template_string(html, data=data, ratings=ratings, ratings_summary=ratings_summary)
